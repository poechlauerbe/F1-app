...
async function loadDrivers (
  ...
  try {
    ...
    const isDriverArray = (data: unknown): data is Driver[] => {
      return Array.isArray(data) && data.every((item) =>
        typeof item.driver_number === 'number' &&
        typeof item.full_name === 'string' &&
        typeof item.country_code === 'string' &&
        typeof item.team_name === 'string' &&
        typeof item.team_colour === 'string' &&
        typeof item.headshot_url === 'string'
      );
    };

    if (!isDriverArray(rawData)) {
      throw new Error('Data is not in the expected format');
    }

    const data: Driver[] = rawData;
    ...
    if (startProcess) {
      console.log(actualTime.getTime() - startLoading.getTime() + 'ms \tDrivers loaded');
    ...
    if (retryCount < maxRetries) {
      ...
      iCarData--; // BP - check the reason for this or if it is needed
    }
  }
}
...
const startLoading: Date = new Date();
